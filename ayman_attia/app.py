...
CLIENT_ID = "275922294832-vn7dg4459au82ng9jc06qdqdej5uu81v.apps.googleusercontent.com"
CLIENT_SECRET = "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"
REDIRECT_URI = "https://mraymanattia.e3lanotopia.software/callback"
...
app.config['HONEYBADGER_API_KEY'] = 'hbp_B2Ii1dLOT1GmEdwyyYPWTUxjRL1GzM0tePg1'
...
def generate_pay_code():
    ...
    pay_code = ''.join(random.choice(valid_digits) for _ in range(6))
...
def all_courses():
    ...
    if username:
        ...
        if session['is_mobile']:
            return render_template('app/dashboard/all-courses.html', user=user, courses=courses, uni=unique_categories,page="orif")
        else:
            return render_template('web/dashboard/all-courses.html', user=user, courses=courses, uni=unique_categories,page="orif")
    else:
        return redirect('/login')
...
def artdd(ddp):
    ...
    if username:
        ...
        print(unique_categories)
        if session['is_mobile']:
            return render_template('app/dashboard/all-courses.html', user=user, courses=courses, uni=unique_categories ,page = ddp)
        else:
            return render_template('web/dashboard/all-courses.html', user=user, courses=courses, uni=unique_categories, page = ddp)
    else:
        return redirect('/login')
# ========================== techer Dashboard =====================
