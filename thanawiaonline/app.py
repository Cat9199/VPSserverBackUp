...
def login():
    if request.method == 'POST':
        ...
        if user:
            if (not session['is_mobile'] or not session['is_webview']) and user.academic_year == "4":
                return render_template('app/login.html',error='ليس مسموح لك باستخدام الموقع قم باستخدام التطبيق',username=identifier)
